...
<template>
  <div>
    <div class="Pad" style="left: 1%;top: 7%;width: 49%;" v-if="!detailShow">
      ...
      <div class="selectedItem" v-for="log in pageLogList" :key="log.id" style="width: 95%;color: gray;display: flex;flex-direction: row;justify-content: flex-start;margin:auto;height: 2rem" id="selectedItem" @click="detailShowHandler(log.id)">
        <div style="width: 18%"><text style="word-wrap: break-word;">{{log.start_time}}</text></div>
        <div style="width: 12%"><text style="word-wrap: break-word;">{{log.target_call_sign}}</text></div>
        <div style="width: 50%"><text style="word-wrap: break-word;">{{log.target_address}}</text></div>
        <div style="width: 10%"><text style="word-wrap: break-word;">{{Math.round(log.distance/1000)}}</text></div>
        <div style="width: 5%">
